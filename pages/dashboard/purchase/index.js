...
export default function Purchase() {
  ...
  const getUserPurchasing = async (data) => {
    fetch(`${BASE_URL}/api/purchase/user`, {
      method: "POST",
      headers: {
    ...
    if (isLoggedInIndication()) {
      let LoggedInUserData = async () => {
        ...
        if (!_.isNull(getuser)) {
          ...
          getUserPurchasing(JSON.parse(getuser));
        }
      };
  ...
  return (
    <>
      <HeadMeta
        title="Dent247 | Dashboard | Purchase History"
        description="description"
        content="Dent247 | Dashboard | Purchase History"
      />
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-2 mb-8 md:mb-6 lg:mb-10 mt-44">
              ...
              <div
                className={`overflow-x-auto ${
                  _.size(purchaseArr) >= 8 ? "tableHeight" : ""
              ...
              >
                <table className="w-full text-sm text-left tableWrapperStyle">
                  ...
                  <tbody>
                    {purchaseArr && purchaseArr?.length > 0 ? (
                      purchaseArr?.map((e) => {
                        return (
              ...
              {!loading && purchaseArr.length > 0 && totalCount > isOffset ? (
