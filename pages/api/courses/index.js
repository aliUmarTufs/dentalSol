...
export default async function handler(req, res) {
  if (req.method === "GET") {
    try {
      ...
      let featuredCoursesList = await supabase
        .from("featured")
        .select("*")
        .eq("is_featured", 1);
      let featuredCourses = _.filter(featuredCoursesList?.data, (item) => {
        return item?.type == "courses";
      });
      let featuredProducts = _.filter(featuredCoursesList?.data, (item) => {
        return item?.type == "products";
      });
      let featuredService = _.filter(featuredCoursesList?.data, (item) => {
      ...
      let relatedArticles = await supabase
        ...
        .eq("article_categories.label", getCourse?.data[0]?.category)
        .limit(10);
      ...
      related_directory = await supabase
        .from("directory_companies")
        .select("* , company(*) , directory_frontend_categories!inner(*) ")
        .eq("directory_frontend_categories.name", getCourse?.data[0]?.category)
        .limit(6);
      // }
      ...
      relatedCourse = await supabase
        .from("courses")
        .select("* , organizations (name)")
        .eq("category", getCourse?.data[0]?.category)
        .not("id", "in", `(${req?.query?.id})`)
        .order("created_at", { ascending: false })
        .limit(10);
      // }
      ...
      if (relatedCourse.data) {
        ...
        let dealsItem = await supabase
          ...
          .eq("is_expire", 0)
          .in("item_id", ids);
        // let featuredItem = await supabase
        //   .from("featured")
        //   .select("*")
        //   .eq("type", "courses")
        //   .eq("is_featured", 1);
      ...
      product = await supabase
        ...
        .eq("product_category.parent_category", getCourse?.data[0]?.category)
        .order("created_at", { ascending: false })
        .limit(10);
      // }
