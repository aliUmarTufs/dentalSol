...
export default function VendorRegistration({ userRoleType }) {
  ...
  const [isValidVendorType, setIsValidVendorType] = useState(true);
  ...
  const validateForm = () => {
    ...
    if (_.isNil(practiceName)) {
      ...
      setPracticeNameErrMsg(REQUIRED_PRACTICE_NAME);
      isValid = false;
    }
    ...
    if (_.isEmpty(vendorTypeData)) {
      vendorTypeRef.current.focus();
      setIsValidVendorType(false);
      isValid = false;
    }
  ...
  const signup = async (event) => {
    ...
    if (validateForm()) {
      ...
      let payload = {
        ...
        userTypeList: _.map(vendorTypeData, "name"),
        username: userName,
      ...
      fetch(`${BASE_URL}/api/auth/signup`, {
        ...
        .then((data) => {
          if (data.status === true) {
            ...
            setTimeout(() => {
              if (data?.data?.role_type === "Vendor") {
                router.push(ROUTES.MULTI_SELECTION);
              } else {
  ...
  useEffect(() => {
    ...
    const vendorTypeFunc = async () => {
      const { data, error } = await supabase.from("user_type_list").select("*");
      if (data) {
        const filteredUserData = _.filter(data, (item) => {
          return item.type.toLowerCase() === ROLE_TYPE[2].title.toLowerCase();
        });
        setVendorType(filteredUserData);
      }
    };
    vendorTypeFunc();
  }, []);
        ...
        <div className="flex items-stretch justify-between gap:2 md:gap-6 flex-col md:flex-row">
          ...
          <div className="flex flex-col gap-2 my-2 flex-1 relative">
            <label
              for="password"
              className={
                "flex items-center text-sm font-normal text-gray-900 dark:text-gray-300"
              }
            >
              Password *{" "}
              <span className="group px-2">
                <InformationCircleIcon className="w-4 h-4" />{" "}
                <span class="group-hover:opacity-100 z-50 transition-opacity bg-gray-600 p-2 text-xs text-gray-100 rounded-md absolute -top-7 left-auto -translate-x-1/2 translate-y-full opacity-0 m-4 mx-auto">
                  {INVALID_PASSWORD}
                </span>
              </span>{" "}
            </label>
        ...
        <div className="flex items-stretch justify-between gap:2 md:gap-6 flex-col md:flex-row">
          ...
          <div className="flex flex-col gap-2 my-2 flex-1">
            ...
            <input
              ...
              placeholder="Type Here"
              onChange={(e) =>
                setPracticeName(e.target.value, setIsValidPracticeName(true))
              }
              value={practiceName}
              ref={practiceNameRef}
            />
          ...
          <div className="flex flex-col gap-2 my-2 flex-1">
            ...
            <input
              ...
              placeholder="Type Here"
              onChange={(e) =>
                setPracticeAdd(e.target.value, setIsValidPracticeAdd(true))
              }
              value={practiceAdd}
              ref={practiceAddRef}
            />
        ...
        <div className="flex items-stretch justify-between gap:2 md:gap-6 flex-col md:flex-row">
          ...
          <div className="flex flex-col gap-2 my-2 flex-1">
            ...
            <input
              ...
              value={orgName}
              onChange={(e) => {
                setOrgName(e.target.value, setIsValidOrgName(true));
              }}
            />

            {!isValidOrgName ? (
              <span className={"text-sm text-red-500"}>{orgNameErrMsg}</span>
            ) : (
              ""
            )}
          </div>
