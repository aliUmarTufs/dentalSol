...
export default function SubNavigation() {
  ...
  if (path == ROUTES.DASHBOARD) {
    home = true;
  } else if (path == ROUTES.DASHBOARD_DEALS || path == ROUTES.DEALS_ADD) {
    deals = true;
  } else if (path == ROUTES.DASHBOARD_WISHLIST ) {
    wishlist = true;
  } else if (path == ROUTES.DASHBOARD_ORGANIZATIONS || path == ROUTES.DASHBOARD_ORGANIZATIONS_ADD) {
    organization = true;
  }
  ...
  const navigation = [
    ...
    { name: "Deals", href: ROUTES.DASHBOARD_DEALS, current: deals },
    { name: "Wishlist", href: ROUTES.DASHBOARD_WISHLIST, current: wishlist },
    // {
    //   name: "Organization",
  ...
  return (
    <Disclosure as="header" className="bg-white shadow">
      {({ open }) => (
        <>
          <div className="max-w-7xl mx-auto mb-8 md:mb-6 lg:mb-10 mt-8 md:mt-12 lg:mt-20">
            <nav
                ...
                <Link href={item.href} key={item.name}>
                  <span
                    ...
                    className={classNames(
                      item.current
                        ? "bg-gray-100 text-gray-900 text-blue-600"
                        : "text-gray-900 hover:bg-gray-50 hover:text-gray-900",
                      "rounded-md py-2 px-4 lg:px-2 inline-flex items-center text-sm font-medium cursor-pointer"
                    )}
